...
common_params_context common_params_parser_init(common_params & params,
    ...
    if (llama_supports_rpc()) {
        add_opt(common_arg({ "--rpc" }, "SERVERS",
                           "comma separated list of RPC servers (tcp://host:port or rdma://host:port)",
                           [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--mmap" }, { "--no-mmap" },
                       string_format("whether to memory-map model (if disabled, slower load but may reduce pageouts if "
                                     "not using mlock) (default: %s)",
    ...
    add_opt(
        common_arg({ "-fitt", "--fit-target" }, "MiB",
                   string_format("target margin per device for --fit option, default: %zu",
                                 params.fit_params_target / (1024 * 1024)),
                   [](common_params & params, int value) { params.fit_params_target = value * size_t(1024 * 1024); })
    ...
    add_opt(common_arg({ "--override-kv" }, "KEY=TYPE:VALUE,...",
                       ...
                       "comma-separated or repeat this argument.\n"
